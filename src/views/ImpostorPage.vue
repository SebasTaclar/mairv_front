  ...
  <div class="impostor-container">
    ...
    <div v-if="showConfirmClearNames" class="popup-overlay">
      <div class="popup-dialog">
        <h2>Ojo!</h2>
        <p class="popup-message">Vas a borrar los nombres de los jugadores, est�s seguro?</p>
        <div class="button-group">
    ...
    <div v-if="currentStep === 'confirmEnd'" class="setup-screen">
      <div class="game-end-card">
        ...
        <p class="end-message">Est�s seguro que deseas terminar el juego?</p>
...
const categoryCatalog: Array<{
  ...
  { key: 'personajes_biblicos', label: 'Personajes bíblicos', section: 'biblical' },
  ...
  { key: 'peliculas', label: 'Películas', section: 'other' },
  { key: 'equipos_de_futbol', label: 'Equipos de fútbol', section: 'other' },
...
const nextCardPreview = () => {
  if (previewOrderIndex.value === playerOrder.value.length - 1) {
    // Todos los jugadores vieron - seleccionar qui�n comienza
    selectNewStart()
...
const nextPlayer = () => {
  if (currentOrderIndex.value === playerOrder.value.length - 1) {
    // �ltimo jugador - confirmar terminar
